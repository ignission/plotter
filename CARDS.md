...
**標準カード**: 30×30×2mm、角 R3
**ワイドカード**: 60×30×2mm、角 R3（修飾キー用、Leader など）

カードは Wedge 上面の長スロット（180mm × 深さ 8mm）に差し込んで立てかけて使う。
スロット内はカードが左右にスライドできる仕切りなし構造なので、標準カードとワイドカードを任意の列に混在させて並べられる。

なお、カード上端のスリット穴（ワイヤ吊り下げ式）は現行設計では採用していない。カードはスロットへの差し込みのみで保持する。
